...
test('drum kit has ten voices; closing the hi-hat chokes only the open hi-hat',async()=>{
 const {audio,ctx}=setup(),inst=INSTRUMENTS.find(i=>i.id==='kit');await audio.ready(inst);assert.equal(inst.notes.length,10);
...
test('duduk has tuned sustained notes, valid smooth loops and monophonic breath release',async()=>{
 ...
 assert.equal(ctx.sources.filter(s=>s.stopTime===undefined).length,1);
...
test('instrument layouts fit their surfaces without overlapping circular pads and every chord has a key',async()=>{
 ...
 assert.equal(KIT_POSITIONS.length,10);for(let i=0;i<10;i++){const [x,y]=KIT_POSITIONS[i];assert.ok(x>=9&&x<=91&&y>=10.08&&y<=89.92);for(let j=0;j<i;j++)assert.ok(Math.hypot(x-KIT_POSITIONS[j][0],(y-KIT_POSITIONS[j][1])/1.12)>18);}
});
