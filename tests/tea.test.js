...
test('tea search combines categories, Russian names and aromas; recipe scales without changing time',()=>{
 ...
 assert.equal(new Set(TEAS.map(t=>t.id)).size,TEAS.length);for(const t of TEAS){assert.equal(t.image,`tea/generated/${t.id}.jpg`);assert.ok(existsSync(`public/${t.image}`));assert.ok(statSync(`public/${t.image}`).size>20000);}
});
