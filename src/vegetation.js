...
export function createVegetation(m,p){
 ...
 function blade(variegated=false,serrated=false,rows=14,cols=6){
  ...
  for(let j=0;j<=rows;j++)for(let k=0;k<=cols;k++){
   ...
   const x=u*span,z=.12*Math.sin(t*Math.PI)-.18*Math.abs(x)-.16*t*t*t;
   vertices.push(x,t,z);uv.push(k/cols,t);
   const vein=Math.abs(u)<.06?1:Math.pow(Math.max(0,Math.cos((t-Math.abs(u)*.2)*Math.PI*18)),16)*.35;
   const speckle=(Math.sin(j*37+k*23)+1)*.5;
   const c=new T.Color(variegated&&Math.abs(u)<.65+speckle*.17?'#c6d298':'#526b38');c.multiplyScalar(.78+vein*.23+speckle*.16);colours.push(c.r,c.g,c.b);
 ...
 const leafMaterial=m.standard('#ffffff',{vertexColors:true,side:T.DoubleSide,roughness:.48});
 const twig=m.standard('#706147');
 function leaf(parent,x,y,z,length,width,a,tilt,material=green){const mesh=new T.Mesh(material==='variegated'?variegatedGeometry:leafGeometry,leafMaterial);mesh.position.set(x,y,z);mesh.rotation.set(tilt,a,(rand()-.5)*1.1);mesh.scale.set(width,length,length);mesh.castShadow=true;parent.add(mesh);return mesh;}
 ...
 for(let j=0;j<19;j++){
  ...
  for(let shoot=0;shoot<3;shoot++){
   ...
   for(let k=0;k<5;k++){const pos=origin.clone().lerp(tip,.1+k*.2);leaf(g,pos.x,pos.y,pos.z,height*(.052+rand()*.022),height*.043,side+k*2.6,1.65+rand()*.65);}
  }
 }
 ...
 const palette=['#e8d157','#f5edcf','#d29cb7','#8c3458','#f0d3dd'];
 ...
 if(j%3===0){cyl(parent,'unopened-dahlia-bud',x,h,z,.023,.018,.055,green,7);continue;}
 ...
 return {potted,flowerbeds,leafGeometry:blade(false,false,8,4)};
}
