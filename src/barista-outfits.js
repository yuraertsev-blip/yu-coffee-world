...
export function dressBarista(vrm,profile,index){
 ...
 vrm.scene.traverse(o=>{if(!o.isMesh)return;const mats=Array.isArray(o.material)?o.material:[o.material];if(mats.every(m=>m.isOutline)){o.visible=false;return;} const convert=m=>{const n=new T.MeshStandardMaterial({name:m.name,map:m.map,color:m.color,roughness:.82,side:m.side,transparent:m.transparent,opacity:m.opacity,alphaTest:m.alphaTest,depthWrite:m.depthWrite});return n;};o.material=Array.isArray(o.material)?mats.map(convert):convert(mats[0]);for(const m of (Array.isArray(o.material)?o.material:[o.material])){
  ...
  if(m.name.includes('SKIN')){m.color.set('#efd1bd');m.roughness=.68;}
  if(m.name.includes('HAIR'))m.roughness=.5;
  if(m.name.includes('Eye'))m.roughness=.27;
 ...
 if(index===1){
  ...
  for(const side of [-1,1])ribbon('apron-edge-stitch',[[side*.127,.80,.132],[side*.127,1.07,.134],[side*.083,1.14,.144],[side*.083,1.23,.151]],.0008,standard('#a4957f'));
 ...
 function attachHead(object,position){attachInModelSpace(vrm.scene,head,object,position);}
 ...
 if(index===0){const crown=new T.Group();crown.name='floral-antler-headband';attachHead(crown,[0,1.60,.005]);
  const band=[];for(let j=0;j<=32;j++){const a=j/32*Math.PI;band.push([Math.cos(a)*.093,Math.sin(a)*.064-.058,.013]);}branch(crown,band,.004,standard('#80663f'));
for(const side of [-1,1]){branch(crown,[[side*.061,0,0],[side*.10,.055,0],[side*.09,.12,0],[side*.11,.19,0]],.0045,gold);branch(crown,[[side*.10,.06,0],[side*.145,.095,0],[side*.15,.13,0]],.003,gold);branch(crown,[[side*.09,.11,0],[side*.055,.14,0]],.003,gold);
   for(let f=0;f<3;f++){const flower=new T.Group();flower.position.set(side*(.052+f*.021),-.012-f*.012,.047-f*.007);crown.add(flower);for(let j=0;j<6;j++){const a=j*Math.PI/3,p=new T.Mesh(new T.SphereGeometry(.012,20,12),f===1?standard('#799cb5'):cream);p.scale.set(1.3,.65,.3);p.rotation.z=a;p.position.set(Math.cos(a)*.008,Math.sin(a)*.008,0);flower.add(p);}const center=new T.Mesh(new T.SphereGeometry(.004,10,6),gold);center.position.z=.003;flower.add(center);}
  }}
