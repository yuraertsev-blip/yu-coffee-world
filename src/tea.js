...
export function installTea({canvas,camera,world,keys,interaction}){
 ...
 const photo=tea=>`<div class="tea-photo"><img src="${assetUrl(tea.image)}" alt="Сухой чай ${tea.name} на чахэ — предметная визуализация" loading="lazy" decoding="async"></div>`;
 function list(){
 ...
 function render(){
  ...
  if(selected){
   dialog.insertAdjacentHTML('beforeend',`<button class="tea-back" data-back>← Вся чайная карта</button><div class="tea-detail"><figure>${photo(selected)}<figcaption>Предметная визуализация · форма листа конкретной партии может отличаться</figcaption></figure><div><span class="tea-eyebrow">${TEA_GROUPS[selected.group]}</span><h2 id="tea-title" tabindex="-1">${selected.name}</h2><p class="tea-taste">${selected.taste}</p><p>${selected.description}</p><section class="tea-brewing"><h3>Заварить без спешки</h3><label class="tea-volume">Объём посуды <select aria-label="Объём посуды"><option value="250">Чашка · 250 мл</option><option value="500">Чайник · 500 мл</option><option value="750">Большой чайник · 750 мл</option></select></label><dl class="tea-recipe"></dl><p class="tea-tip"></p>${selected.portion?'<p class="tea-small">Это рецепт по весу листа. Целая порция может весить больше — проверьте упаковку.</p>':''}<p class="tea-small">Базовый способ настаивания, не короткие проливы. Регулируйте крепость по вкусу; лист после заваривания отделите от настоя.</p></section></div></div>`);
