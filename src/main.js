...
const scene=new T.Scene();scene.background=new T.Color('#c8d2df');scene.fog=new T.Fog('#c8d2df',65,140);
...
const ambient=new T.HemisphereLight('#dce7ff','#8a877b',1.3);scene.add(ambient);
const sun=new T.DirectionalLight('#fff5e3',2.6);sun.position.set(-18,26,20);sun.castShadow=true;sun.shadow.mapSize.set(2048,2048);Object.assign(sun.shadow.camera,{left:-35,right:35,top:30,bottom:-30,near:1,far:80});sun.shadow.normalBias=.035;scene.add(sun);
...
$('#light').onclick=()=>{night=!night;$('#light').setAttribute('aria-pressed',String(night));$('#light').innerHTML=night?'☾ <span>Вечер</span>':'☀ <span>День</span>';scene.background.set(night?'#465164':'#c8d2df');scene.fog.color.copy(scene.background);ambient.intensity=night?.45:1.3;sun.intensity=night?.18:2.6;world.lamps.forEach(l=>l.intensity=night?12:9);};
